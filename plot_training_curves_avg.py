...
def plot_averaged(
    ...
    for algo in ALGOS:
        ...
        ax.plot(x_grid, mean, color=color, label=f"{ALGO_LABELS[algo]} (n={n_seeds} seeds)")
        ax.fill_between(x_grid, mean - std, mean + std, color=color, alpha=0.2)

        if env_id:
            envs_plotted.add(env_id)
    ...
    ax.set_title(f"Training reward, averaged over seeds ({timesteps:,} steps)")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Welches DQN-Env in den Vergleich soll. "
...
def main():
    ...
    plot_averaged(Path(args.models_dir), args.timesteps, args.seeds, Path(args.out), env_overrides)
