...
def plot_checkpoint_progression(all_ckpt_rows: dict[str, list[dict]], out_path: Path) -> None:
    """Tracks Q-value bias and re-eval performance across every saved
    checkpoint of the three 1M-timestep runs (one per algo), giving a much
    finer-grained view of the training trajectory than the 3-point
    (10k/100k/1M) approximation used elsewhere.
    """
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 5))

    for algo, rows in all_ckpt_rows.items():
        rows = sorted(rows, key=lambda r: r["checkpoint_steps"])
        if not rows:
            continue
        steps = [r["checkpoint_steps"] for r in rows]
        ax1.plot(steps, [r["relative_bias"] for r in rows], marker="o", label=algo.upper())
        ax2.plot(steps, [r["reeval_mean"] for r in rows], marker="o", label=algo.upper())

    ax1.axhline(0, color="gray", linestyle="--", linewidth=0.8)
    ax1.set_xlabel("Timesteps")
    ax1.set_ylabel("Relative Q-value bias (bias / |mc_return_mean|)")
    ax1.yaxis.set_major_formatter(lambda y, _: f"{y:.0%}")
    ax1.set_title("Q-value bias over training\n(1M runs, every checkpoint)")
    ax1.legend()
...
def print_checkpoint_summary(all_ckpt_rows: dict[str, list[dict]]) -> None:
    for algo, rows in all_ckpt_rows.items():
        rows = sorted(rows, key=lambda r: r["checkpoint_steps"])
        print(f"\n=== {algo.upper()} checkpoint progression (1M run) ===")
        for row in rows:
            print(
...
def run_checkpoints_mode() -> None:
    """Evaluates every saved checkpoint of the three 1M-timestep runs (one
    per algo: DQN/CartPole, SAC/Pendulum, TD3/Pendulum) and plots how bias
    and performance evolve over the course of training.
    """
    run_dirs_1m = []
    ...
    ckpt_rows_by_algo: dict[str, list[dict]] = {}
    all_ckpt_rows: list[dict] = []
    for run_dir in sorted(run_dirs_1m):
        with open(run_dir / "config.json") as f:
            algo = json.load(f)["algo"]
        rows_for_run = evaluate_checkpoints(run_dir)
        if not rows_for_run:
            print(f"Warnung: keine Checkpoints gefunden in {run_dir / 'checkpoints'}")
            continue
        ckpt_rows_by_algo[algo] = rows_for_run
        all_ckpt_rows.extend(rows_for_run)

    write_checkpoint_csv(all_ckpt_rows, RESULTS_DIR / "checkpoint_evaluation.csv")
    plot_checkpoint_progression(ckpt_rows_by_algo, RESULTS_DIR / "finding_checkpoint_progression.png")

    print_checkpoint_summary(ckpt_rows_by_algo)
    print(f"\nCSV: {RESULTS_DIR / 'checkpoint_evaluation.csv'}")
    print(f"Plot: {RESULTS_DIR / 'finding_checkpoint_progression.png'}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Evaluate every saved checkpoint (models/*/checkpoints/model_*_steps.zip) "
             "of the three 1M-timestep runs (one per algo) instead of the final/best "
