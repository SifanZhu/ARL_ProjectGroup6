...
```
Input (3) → Linear(128) → ReLU → Linear(128) → ReLU → Linear(N_ACTIONS)
```
...
| `N_EPISODES` | 300 | Training episodes per seed |
...
Training is repeated across 3 independent seeds `[42, 0, 1]`. The plot shows:
- Faint lines: individual seed traces
- Shaded band: min/max range across seeds
- Solid line: smoothed mean (window = 20 episodes)

Typical final performance (last 50 episodes, mean across seeds): **~ −150 to −160**
...
3. Record a GIF of the best-performing agent and save it as `dqn_pendulum.gif`
